...
function findConfigJson(basedir) {
  const packagePath = path.resolve(basedir, ".js-hyperclick.json")
  try {
    fs.accessSync(packagePath)
  } catch (e) {
    const parent = path.resolve(basedir, "../")
...
function findConfigPath(basedir) {
  const packagePath = findConfigJson(basedir)
  return packagePath ? packagePath : findPackageJson(basedir)
}

function loadModuleRoots(basedir) {
  const packagePath = findConfigPath(basedir)
  if (!packagePath) {
    return
  }
